...
const HAS_CREDS = !!(creds.aq.apiKey && creds.dhan.accessToken);
const describeIf = HAS_CREDS ? describe : describe.skip;

describeIf('Dhan Broker Integration', () => {
  let headers;
  ...
  test('fetch holdings', async () => {
    const result = await api.getHoldings('dhan', {
      clientId: creds.dhan.clientId,
      accessToken: creds.dhan.accessToken,
    }, headers);
    expect(result).toBeDefined();
  });

  test('fetch funds/balance', async () => {
    const result = await api.getFunds('dhan', {
      clientId: creds.dhan.clientId,
      accessToken: creds.dhan.accessToken,
    }, headers);
    expect(result).toBeDefined();
  ...
  test('fetch order book', async () => {
    const result = await api.getOrderBook('dhan', {
      clientId: creds.dhan.clientId,
      accessToken: creds.dhan.accessToken,
    }, headers);
    expect(result).toBeDefined();
