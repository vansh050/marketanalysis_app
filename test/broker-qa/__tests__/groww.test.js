...
const HAS_CREDS = !!(creds.aq.apiKey && creds.groww.accessToken);
const describeIf = HAS_CREDS ? describe : describe.skip;

describeIf('Groww Broker Integration', () => {
  let headers;
  ...
  test('fetch holdings', async () => {
    const result = await api.getHoldings('groww', {
      accessToken: creds.groww.accessToken,
    }, headers);
    expect(result).toBeDefined();
  });

  test('fetch funds/balance', async () => {
    const result = await api.getFunds('groww', {
      accessToken: creds.groww.accessToken,
    }, headers);
    expect(result).toBeDefined();
  });
  ...
  test('fetch order book', async () => {
    const result = await api.getOrderBook('groww', {
      accessToken: creds.groww.accessToken,
    }, headers);
    expect(result).toBeDefined();
  });
