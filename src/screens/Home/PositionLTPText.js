...
const PositionLTPText = React.memo(({ symbol, advisedRangeCondition, advisedPrice, exchange, type }) => {
  ...
  const backgroundColor = missedGainPercentage != null && missedGainPercentage > 0 ? 'green' : 'red';

  return (
    <View>
      <View style={styles.priceContainer}>
        <Text style={styles.price}>LTP: ₹{price}</Text>
      </View>
    </View>
  );
});
...
export default PositionLTPText;